...
public static class ScryCmd
{
    ...
    {
        // 本版本没有独立的 Scry 变量; 预视张数借用 Cards 变量承载(卡面写作"预视 N")。
        return Execute(choiceContext, card.Owner, card.DynamicVars.Cards.IntValue);
    }

    public static async Task<ScryResult> Execute(PlayerChoiceContext choiceContext, Player player, int amount)
    {
        var modifiedAmount = yylHook.ModifyScryAmount(player, amount, out var modifiers);
        await yylHook.AfterModifyingScryAmount(choiceContext, player, modifiers, amount, modifiedAmount);
        ...
        await yylHook.AfterScryed(choiceContext, player, modifiedAmount, cardsToDiscard.Count, cardsToDiscard);
